	...
	const FFramePackage& YoungerFrame, float HitTime)
{
	...
	const float InterpFraction = FMath::Clamp((HitTime - OlderFrame.Time) / Distance, 0.f, 1.f); // This will be the interp speed
	...
	const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocity, float HitTime)
{
	...
	else // we didn't hit the head, check other boxes
	{
		for(auto& HitBoxPair : HitCharacter->HitCollisionBoxes)
		{
			if (HitBoxPair.Value != nullptr)
                                                                            ...
                                                                            const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations)
{
	...
	for (auto& Frame : FramePackages) // The FramePackages would have already been interpolated at this time
	{
		FFramePackage CurrentFrame;
		CurrentFrame.Character = Frame.Character;
		CacheBoxPositions(Frame.Character, CurrentFrame); // The current frame is the package that we save in the present 
		MoveBoxes(Frame.Character, Frame); // The Frame is the package where we got a hit (it can be in the past)
	...
	// enable collision for all boxex, then disable for headbox
	for (auto& Frame : FramePackages)
	{
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	const TDoubleLinkedList<FFramePackage>& History = HitCharacter->GetLagCompensation()->FrameHistory; // avoid copying and avoid changing value because we use reference
	const float OldestHistoryTime = History.GetTail()->GetValue().Time;
	const float NewestHistoryTime = History.GetHead()->GetValue().Time;
	if (OldestHistoryTime > HitTime)
	{
		// too far back - too laggy to do ServerSideRewind(SSR)
	...
	while (Older->GetValue().Time > HitTime) // is Older still younger than HitTime?
	{
		...
		if (Older->GetNextNode() == nullptr) break; //incase the older node is a tailnode than that means nextnode willbe null
...
void ULagCompensationComponent::SaveFramePackage(FFramePackage& Package)
{
	...
	if (Character)
	{
		Package.Time = GetWorld()->GetTimeSeconds(); //This is the servers time and all the clients know the server's time (From BlasterPlayerController)
