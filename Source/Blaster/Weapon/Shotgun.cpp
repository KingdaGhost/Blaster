...
{
	AWeapon::Fire(FVector());// We didn't use Super because we want to use only the Fire function of the weapon class
	...
	if (MuzzleFlashSocket)
	{
		...
		for (auto HitPair : HitMap)
		{
			if (HitPair.Key)
			{
				...
				HitCharacters.AddUnique(HitPair.Key); // only one character will be added if it hits the same character multiple times			
			}
		}
